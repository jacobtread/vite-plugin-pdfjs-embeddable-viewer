...
interface PdfjsEmbeddableViewerOptions {
  /**
   * Whether to use the legacy browser supporting
   * version of the pdf viewer
   */
  legacy?: boolean;
  ...
  /**
   * Whether to patch out the cross origin blocking and allow cross origin access
   */
    ...
    ...viteStaticCopy({
      targets: [
        ...
        // Patch viewer.html
        {
          ...
          dest: outputPath,
          transform: (content, _fileName) => {
            const disableJs = config.disableJs ?? false;
            const customOptions = config.openOptions !== undefined;
